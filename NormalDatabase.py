...
def checkValidDataType(data):
    data = data.strip()  # Clear off trailing whitespace
    # The only recognized types are "Numeric" and "Categorical"
    if data != "Numeric" and data != "Categorical" and data!= "Categorical\r":
...
class NormalDatabase:
    ...
    def normalizeNumericData(self):
        for dimension in range(self.dimension):     # Go through each dimension
            ...
            elif self.instructions.components[dimension] == "Dismiss" or \
                    self.instructions.components[dimension] == "Target":
    ...
    def toCSV(self):
        with open("normDB.csv", "w") as file:
            mywriter = csv.writer(file, delimiter=',')
            mywriter.writerow(self.validComponents(self.labels.components))
            for vector in self.training.components:
                mywriter.writerow(self.noZeros(vector.components))
        py_to_r.kmcEigen("normDB.csv")

    def zeroDimension(self, n):
        for entry in self.training.components:
            entry.components[n] = 0

    def noZeros(self, vector):
        newVector = []
        for component in vector:
            if component != 0:
                newVector.append(component)
        return newVector

    def validComponents(self, vector):
        newVector = []
        for component in range(len(vector)):
            if self.instructions.components[component] == "":
                newVector.append(component)
        return newVector
